#!/bin/bash
# Run A5 and A6 experiments (pred_len=96 only)
...
# Usage: cd Forecasting && bash run_a5_a6.sh
...
LATEST=$(ls -td saved_models/*prob_twostage*pl96* 2>/dev/null | head -1)
[ -n "$LATEST" ] && python test.py --model_dir "$LATEST"

echo "A5 and A6 experiments completed!"
