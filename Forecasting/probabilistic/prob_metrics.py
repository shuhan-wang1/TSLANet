...
def compute_nll(mu, log_var, target):
    """
    Gaussian negative log-likelihood (with constant term).
...
def compute_calibration(mu, total_var, target, quantiles=None):
    """
    Compute calibration: for each nominal coverage level p,
    compute the fraction of true values falling within the p% prediction interval.

    Args:
        mu: (N, T, M) predicted means
        total_var: (N, T, M) total predicted variance
        target: (N, T, M) ground truth
        quantiles: list of nominal coverage levels (default: 10% to 95%)
    ...
    for q in quantiles:
        # z-score for symmetric interval with coverage q
        # P(-z < Z < z) = q => Phi(z) = (1+q)/2 => z = Phi^{-1}((1+q)/2)
...
def compute_sharpness(total_var, confidence=0.9):
    """
    Average width of the prediction interval at given confidence level.
    Width = 2 * z * sigma where z = Phi^{-1}((1+confidence)/2).
    """
...
def compute_pit_values(mu, total_var, target):
    """
    Probability Integral Transform: Phi((y - mu) / sigma).
    Should be Uniform(0,1) for a well-calibrated Gaussian model.

    Returns: (N*T*M,) flattened PIT values
    """
    sigma = torch.sqrt(total_var + 1e-8)
    z = (target - mu) / sigma
    pit = 0.5 * (1.0 + torch.erf(z / math.sqrt(2)))
    return pit.flatten()
...
def compute_all_metrics(results_dict):
    ...
    Args:
        results_dict: dict from mc_dropout_predict / deep_ensemble_predict / deterministic_predict
            Required keys: mu_mean, targets, total_var, epistemic_var, aleatoric_var
    ...
    # Log-var for NLL/CRPS computation
